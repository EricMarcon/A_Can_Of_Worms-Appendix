...
author:
  - name: Thibaud Decaëns
  - name: George G. Brown
  - name: Erin K. Cameron
  - name: Nico Eisenhauer
  - name: Sylvain Gérard
...
Packages <- c("tidyverse", "cli", "HDInterval", "ggcorrplot")
# Install them
...
#' @param model The model to estimate: "Joppa", "NoProgress"
...
ll_prior <- function(theta) {
  ...
  sum(
    ...
    dunif(theta[6], min = 0, max = 10, log = TRUE)        # sigma    
  )
}
```
...
Our choice is to keep an estimation out of 5000.
...
#### Posterior likelihood and correlations

```{r}
# Save the results
posterior_joppa <- as_tibble(posterior[, -(3:4)])
...
We keep an estimation out of 5000.
...
# Estimating regional diversity and the taxonomic deficit
...
posterior_joppa %>% 
  ...
  geom_density(
    ...
    adjust = bw.SJ(posterior_joppa$S_T)/bw.nrd0(posterior_joppa$S_T),
    bounds = c(6000, Inf)
  ) +
  xlim(6000, 12500) +
  ...
  labs(
    title = "A", 
    x = bquote('Number of Species'~(italic(S[T]))), 
